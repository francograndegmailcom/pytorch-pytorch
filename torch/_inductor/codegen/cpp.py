...
class CppKernel(Kernel):
    ...
    def store_reduction(self, name, index, value):
        ...
        self.reduction_suffix.writeline(
            DeferredLine(
                name,
                PlaceHolderLine(value, lambda x: f"{var}[{cexpr_index(index)}] = {x};"),
            )
        )
    ...
    def aggregate_reduction_buffers(self, loop_nest, outer_loop_level):
        ...
        def extend_outer_loop_reduction():
            ...
            for k, v in self.scalar_kernel.reduction_cse.reduction_cache.items():
                ...
                # we will visit arr variable later by tmp_acc[tail - tiling_offset] in stores and reduction_suffix
                # since they are PlaceHolderLine, we can update the name directly here
                v.name = f"{v.name}_arr[{outer_loop.var}_tail - {cexpr_index(outer_loop_offset)}]"
