...
def _rewrite_node(gm: torch.fx.GraphModule):
    """
...
def _get_module_call_graph(export_artifact: ExportArtifact, original_in_spec: TreeSpec, preserve_module_call_signature: Tuple[str, ...], strict_mode_export: bool):
    """
...
def _get_range_constraints(export_artifact: ExportArtifact, combined_args: Dict[str, Any], dynamic_shapes):
    gm: torch.fx.GraphModule = export_artifact.aten.gm
    export_graph_signature: ExportGraphSignature = export_artifact.aten.sig
...
def _export_for_training(
    ...
    *,
    ...
    args, kwargs, orig_in_spec, original_state_dict, dynamic_shapes = _process_export_inputs(mod, args, kwargs, dynamic_shapes)
    ...
    range_constraints = _get_range_constraints(
        export_artifact,
        _combine_args(
            mod, args, kwargs, _is_torch_jit_trace=False
        ),
        dynamic_shapes,
    )
    # The returned the gm is in-place modified 
    gm, module_call_graph = _get_module_call_graph(export_artifact, orig_in_spec, preserve_module_call_signature, strict)
    ...
    gm.meta["inline_constraints"] = inline_constraints
...
def _export(
    ...
    *,
    ...
    args, kwargs, original_in_spec, original_state_dict, dynamic_shapes = _process_export_inputs(mod, args, kwargs, dynamic_shapes)
    ...
    forward_arg_names =  _get_forward_arg_names(mod, args, kwargs) if not _is_torch_jit_trace else None
    inline_constraints = _get_inline_constraints(export_artifact.fake_mode)
    range_constraints = _get_range_constraints(
        export_artifact,
        _combine_args(
    ...
    gm, module_call_graph = _get_module_call_graph(export_artifact, original_in_spec, preserve_module_call_signature, strict)
